...
class Adxl34x(object):
    ...
    def __init__(self):
        # Creat logger
        self.__logger = mlog.MatrixLogger("Adxl34x")
        ...
        try:
            for item in os.listdir(sys_dir):
                ...
                if os.path.exists(position):
                    ...
                    break;
            if not found:
                self.__logger.warn("error, adxl34x position not found.")
                return -1
        except OSError, e:
            self.__logger.error("error, OSError.")
            return -1

        try:
            fd = open(position, 'r')
        except IOError, e:
            self.__logger.error("error: fail to open position")
            return -1
        else:
