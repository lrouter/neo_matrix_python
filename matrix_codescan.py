...
class Codescan(object):
    ...
    def __init__(self):
        ...
        # Creat serial port
        self.__serialComPort = serial.Serial('/dev/ttyS1', 9600, timeout=5)
    ...
    ####################################################
    ##  从设备读取数据
    ##  在5秒内一次性读取50个字节的数据。如果5秒时间到但是没有数据，会返回空。
    ##  如果5秒内只收到了30个字节的数据，会返回30个字节的数据。
    ####################################################
    def readData(self):
        ackLen = 50
        data = self.__serialComPort.read(ackLen)
        return data
