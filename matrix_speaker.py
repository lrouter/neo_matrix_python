...
class Speaker(object):
    ...
    def __init__(self):
        # Creat Log
        self.__logger = mlog.MatrixLogger("Speaker")

        # Creat serial port
        self.__ser = serial.Serial('/dev/ttyS2', 9600, timeout=0.5)
        if not self.__ser.isOpen():
            self.__logger.error('self.__serial /dev/ttyS2 could not be opened.')
            return False
    ...
    def setVolume(self, value):
        ...
        if (value > 30 or value < 0):
            self.__logger.error('Error Paramter: volume should be 0 ~ 30, %d', value)
            return -1
        list.append(value)
    ...
    def setEQ(self, value):
        ...
        if (value > 5 or value < 0):
            self.__logger.error('Error Paramter: EQ should be 0 ~ 5, %d', value)
            return -1
        list.append(value)
    ...
    def setPlayMode(self, value):
        ...
        if (value > 4 or value < 0):
            self.__logger.error('Error Paramter: play mode should be 0 ~ 4, %d', value)
            return -1
        list.append(value)
    ...
    def setMusicNum(self, value):
        ...
        if (value > 255 or value < 1):
            self.__logger.error('Error Paramter: musci number should be 1 ~ 255, %d', value)
            return -1
        vh = value >> 8
        vl = value & 0xff
        list.append(vh)
